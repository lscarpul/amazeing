...
class GeneratoreLabirinto:
    ...
    def __init__(self, larghezza: int, altezza: int, seme: int = None):
    ...
    def _inizializza_griglia(self):
        ...
        self.griglia = [[Cella(x, y) for x in range(self.larghezza)] for y in range(self.altezza)]

    def get_grid(self):
        """Restituisce la griglia del labirinto, convertita in formato compatibile con a_maze_ing."""
        # Mantengo compatibilità con l'oggetto Cell e le chiavi in inglese attese fuori
        class WrapperCella:
    ...
    def generate(self, start_x: int = 0, start_y: int = 0) -> None:
        ...
        if cella_partenza is None:
            print("Attenzione: Non è stato possibile trovare una cella di partenza valida.")
            return
    ...
    def _trova_cella_partenza_valida(self, start_x: int, start_y: int) -> Cella:
    ...
    def _creerebbe_area_aperta(self, cella_attuale: Cella, cella_vicina: Cella) -> bool:
        ...
        if dx != 0:
            ...
            if cella_attuale.y > 0 and cella_vicina.y > 0:
                adiacente_attuale = self.griglia[cella_attuale.y - 1][cella_attuale.x]
                adiacente_vicina = self.griglia[cella_vicina.y - 1][cella_vicina.x]
                # Se il muro sopra della cella_attuale e della cella_vicina è aperto, E il muro tra le adiacenti è aperto
                if not cella_attuale.muri['N'] and not cella_vicina.muri['N'] and not adiacente_attuale.muri['E' if dx == 1 else 'W']:
                    return True
            # Controlla sotto
            if cella_attuale.y < self.altezza - 1 and cella_vicina.y < self.altezza - 1:
                adiacente_attuale = self.griglia[cella_attuale.y + 1][cella_attuale.x]
                adiacente_vicina = self.griglia[cella_vicina.y + 1][cella_vicina.x]
                if not cella_attuale.muri['S'] and not cella_vicina.muri['S'] and not adiacente_attuale.muri['E' if dx == 1 else 'W']:
                    return True
        ...
        if dy != 0:
            ...
            if cella_attuale.x > 0 and cella_vicina.x > 0:
                ...
                if not cella_attuale.muri['W'] and not cella_vicina.muri['W'] and not adiacente_attuale.muri['S' if dy == 1 else 'N']:
                    return True
            # Controlla a destra
            if cella_attuale.x < self.larghezza - 1 and cella_vicina.x < self.larghezza - 1:
                adiacente_attuale = self.griglia[cella_attuale.y][cella_attuale.x + 1]
                adiacente_vicina = self.griglia[cella_vicina.y][cella_vicina.x + 1]
                if not cella_attuale.muri['E'] and not cella_vicina.muri['E'] and not adiacente_attuale.muri['S' if dy == 1 else 'N']:
                    return True
    ...
    def _abbatti_muro(self, cella_a: Cella, cella_b: Cella):
        ...
        if dx == 1:
            cella_a.muri['E'] = False; cella_b.muri['W'] = False
        elif dx == -1:
            cella_a.muri['W'] = False; cella_b.muri['E'] = False
        elif dy == 1:
            cella_a.muri['S'] = False; cella_b.muri['N'] = False
        elif dy == -1:
            cella_a.muri['N'] = False; cella_b.muri['S'] = False
    ...
    def _vicini_non_visitati(self, cella: Cella) -> list[Cella]:
        ...
        if cella.x < self.larghezza - 1 and not self.griglia[cella.y][cella.x + 1].visitata:
            vicini.append(self.griglia[cella.y][cella.x + 1])
        if cella.y < self.altezza - 1 and not self.griglia[cella.y + 1][cella.x].visitata:
            vicini.append(self.griglia[cella.y + 1][cella.x])
    ...
    def solve(self, entrata: tuple[int, int], uscita: tuple[int, int]) -> bool:
        ...
        # Insieme per tenere traccia delle celle già esplorate per evitare cicli infiniti
        visitate = {cella_partenza}
        
        while coda:
            cella_corrente, percorso_corrente = coda.popleft()

            # Se abbiamo raggiunto l'uscita, salviamo il percorso come stringa e terminiamo
            ...
            for direzione, vicino in vicini_aperti.items():
                if vicino not in visitate:
                    visitate.add(vicino)
                    # Aggiunge il vicino alla coda con il nuovo percorso aggiornato
